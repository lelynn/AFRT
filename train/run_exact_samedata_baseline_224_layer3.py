...
class Alexnet(HybridBlock):
    ...
    def __init__(self, layer: int, ctx) -> None:
        ...
        # self.load_parameters("alexnet-44335d1f.params", ctx=ctx, ignore_extra=True)
...
class AFRT(HybridSequential):
    ...
    def __init__(self, layer: int, ctx) -> None:
        ...
        with self.name_scope():
            self.add(Affine(self.selected_fov))
            self.add(Response())
...
if __name__ == '__main__':
    train_path = '../../projects/bin/monkey_b2p/THINGS_train'
    test_path = '../../projects/bin/monkey_b2p/THINGS_test'
    ...
    for roi in [
        ...
        for i,c in enumerate(new_ind):
            ...
            for layer in [
                ...
                # Define model
                fov = [224, 224, 224, 224, 224, 224, 224, 224]
                selected_fov = fov[layer - 1]
                transformer = gluon.data.vision.transforms.Resize(size=(selected_fov, selected_fov))
                ...
                for epoch in tqdm(range(n_epochs), total = n_epochs):
                    ...
                    for batch in training_iterator:
                        ...
                        with autograd.record():
                            x_train = transformer(batch.data[0].transpose((0,2,3,1))).transpose((0,3,1,2))
                    ...
                    for batch in testing_iterator:
                        x_test = transformer(batch.data[0].transpose((0,2,3,1))).transpose((0,3,1,2))
